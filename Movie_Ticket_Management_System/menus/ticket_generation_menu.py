...
def ticket_generation_menu():
    while True:
        console.print(Panel.fit("[bold cyan] TICKET GENERATION[/bold cyan]", border_style="cyan"))
        ...
        if choice == "1":
            ...
            console.print(Panel.fit("[bold green]Thank you for using Ticket Generation. Have a great day![/bold green]", border_style="green"))
            break
        else:
...
def generate_ticket_menu():
    try:
        ...
        if not booking_id or not booking_id.isdigit():
            console.print("[bold red]Please enter a valid numeric Booking ID.[/bold red]")
            return

        ticket = generate_ticket(int(booking_id))
        if ticket is None:
            console.print("[bold yellow]No booking found for this Booking ID.[/bold yellow]")
            return

        display_ticket(ticket)
    except Exception as e:
        console.print(f"[bold red]Unable to generate ticket: {e}[/bold red]")

def display_ticket(ticket):
    table = Table(title=" MOVIE TICKET", show_header=False, border_style="cyan")
    ...
    console.print()
    console.print(table)
    console.print("[bold green]Ticket generated successfully![/bold green]")